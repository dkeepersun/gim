...
func wsHandler(w http.ResponseWriter, r *http.Request) {
	appId, _ := strconv.ParseInt(r.URL.Query().Get(grpclib.CtxAppId), 10, 64)
	userId, _ := strconv.ParseInt(r.URL.Query().Get(grpclib.CtxUserId), 10, 64)
	deviceId, _ := strconv.ParseInt(r.URL.Query().Get(grpclib.CtxDeviceId), 10, 64)
	token := r.URL.Query().Get(grpclib.CtxToken)
	requestId, _ := strconv.ParseInt(r.URL.Query().Get(grpclib.CtxRequestId), 10, 64)
